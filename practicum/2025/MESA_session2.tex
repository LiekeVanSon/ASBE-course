...
% \subsection{Customizing Output Quantities}

% MESA determines output columns using:
% \begin{verbatim}
% $MESA_DIR/star/defaults/history_columns.list
% $MESA_DIR/star/defaults/profile_columns.list
% \end{verbatim}

% To customize, copy these files into your work directory:
% \begin{verbatim}
% cp $MESA_DIR/star/defaults/history_columns.list .
% cp $MESA_DIR/star/defaults/profile_columns.list .
% \end{verbatim}

% Edit them to comment or uncomment variables of interest (lines starting with \texttt{!} are ignored). For advanced output, use the \texttt{run\_star\_extras.f} file, discussed in Part 2.

% \section{Plotting MESA Output with \texttt{mesa\_reader}}

% For high-quality plots, post-processing with Python is recommended. The \texttt{mesa\_reader} module is a lightweight Python tool requiring only \texttt{numpy}.
...
% \subsection{Example: Temperature–Density Profile}
