...
\begin{itemize}
    \item[$\rightarrow$] Download the MESA inlists needed for this tutorial from Brightspace (\texttt{session3.zop}). \\
...
There is no need to wait for models to finish before starting on the analysis and plotting code!
\end{tcolorbox}
...
You need \texttt{hw3\_inlist\_problem2} for this exercise. This inlist results in more properly resolved stars (note the difference in HR diagram). Because of this it will take much longer run the late evolution, which is why we will only run the stars until core He exhaustion.
