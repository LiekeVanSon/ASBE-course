...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Luminosity of Massive Stars at late stages}
...
\begin{itemize}
    ...
    \item ./clean; ./mk the directory, and ./rn  MESA models at $M = 20, 30, 40 M_\odot$ through central C exhaustion. That is, change in the \texttt{\&controls} section in \texttt{hw3\_inlist}:
        \begin{lstlisting}
...
\begin{tcolorbox}[protipbox]
You will be making quite a few HR diagrams in this exercise. Consider creating a reusable Python function to streamline the plotting process.
\end{tcolorbox}

\begin{enumerate}[label=(\alph*)]
    \item Plot the evolutionary tracks of the three massive stars on a single HR diagram as scatter plots, each colored by a different luminosity component: $\log(L_i / L)$ for $i =$ H, He, and Z.

    Clip the pre-main sequence evolution from your plot for clarity. For example, start from the first step with central hydrogen abundance $<0.69$:
...
print(hist.star_age[start:])
    ...
    \item Create a fourth plot. This will again be an HR diagram, but instead of phothon luminosity on the y-axis, use the luminosity emitted in neutrinos (\texttt{log\_Lneu})
    Color this plot by \texttt{np.log10(center\_c12)}.
...
%
You need \texttt{hw3\_inlist\_problem2} for this exercise. This inlist results in more properly resolved stars (note the difference in HR diagram). Because of this it will take much too long to run the late evolution, which is why we will only run the stars until core He exhaustion.

% \todo{finally: only run models between 15-40Msun and DW between 0 and 1!}

\begin{itemize}
    \item Choose a mass between 15 M$_\odot$ and 40 M$_\odot$, and change the inlist accordingly. Don't forget to point the master inlist to \texttt{hw3\_inlist\_problem2} instead of \texttt{hw3\_inlist\_to\_C\_exhaust}
    
    \item Run the same mass model with 3 different mass-loss rates. \\
    To do this vary the \verb|Dutch_scaling_factor| between 0 and 1.5, (for example 0.1, 0.5, and 1.5). 
...
\item Investigate \emph{surface abundances} changes due to mass-loss: make separate plots of the surface abundances of H, He, C and O vs age, one for each stellar model. Can you explain the changes you see, and their dependence on the mass-loss rate?
%%%%%%%%%% surf ab questions
...
Using your model for the star you picked with a mass between 15 and 100 M$_\odot$ from the previous exercise (with \verb|Dutch_scaling_factor| your middle value) :

\begin{enumerate}[label=(\alph*)]
\item Make a plot of the radius of the star versus the age. 
\end{enumerate}

\begin{itemize}
    \item What are the fast and slow phases of evolution? When does the star expand, and when does it contract? By how much does the star expand during its evolution?
\end{itemize}
