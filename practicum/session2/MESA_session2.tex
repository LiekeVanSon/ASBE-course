...
\begin{enumerate}
  \item \textbf{Create a virtual environment} by running the following command in your home directory using the terminal:
\begin{lstlisting}
...
\end{lstlisting}
    You will notice that your terminal prompt changes adding \texttt{(myenv)} to the stars, indicating that you are now working inside the virtual environment. To deactivate the virtual environment, simply run: \texttt{deactivate}.

    \item While inside your virtual environment, you can install Mesa Reader by running:
\begin{lstlisting}
...
\end{lstlisting}
    ...
    If for some reason you encounter trouble with the pip install, you can alternatively directly git clone (download) the code from the project's \href{https://github.com/wmwolf/py_mesa_reader}{Github repository} somewhere in your \texttt{home} directory (e.g. \verb|~/codes|)\footnote{Note that the symbol \texttt{\~} points to your home directory.} by running:
\begin{lstlisting}
...
\item
  Throughout this course, we will use \texttt{Jupyter Notebook} to analyse our results. 
  If you have correctly copied your 
  This is accessed via \url{https://jupyterhub.science.ru.nl/} and logging in with your science account.
